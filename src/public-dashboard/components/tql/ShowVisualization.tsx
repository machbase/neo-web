...
export const ShowVisualization = (props: ShowChartProps) => {
    ...
    const LoadCodeScripts = async () => {
        ...
        // Public dashboard: Load and modify jsCodeAssets URLs
        if (sCodeAsset && sCodeAsset.length > 0) {
            for (const url of sCodeAsset) {
                try {
                    const response = await fetch(url);
                    if (response.ok) {
                        let jsCode = await response.text();
                        // Replace API URLs with current server + /db/tql
                        const currentServerUrl = `${window.location.protocol}//${window.location.host}`;
                        jsCode = jsCode.replace(/["'][^"']*\/web\/api\/tql["']/g, `"${currentServerUrl}/db/tql"`);
    ...
    /**
     * Synchronous fallback capture used at clear sites. The
     * 'legendselectchanged' event listener is the primary capture path; this
     * helper exists so we never lose state if a clear() happens between
     * renders before the handler ran. Reads legend.selected directly from
     * getOption() — best-effort, silently no-ops on a disposed/empty instance.
     */
    ...
    const RestoreLegendSelection = () => {
        ...
        try {
            ...
            // / series / xAxis here so other overlays (e.g. graphic) survive.
