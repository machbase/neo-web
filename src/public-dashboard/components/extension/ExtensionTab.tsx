...
import { VscCheck, VscPass } from 'react-icons/vsc';
...
const Table = ({
    ...
    const cellRenderers = replaceCell?.key
        ? [{ column: replaceCell.key, render: (row: any[]) => <span>{replaceCell.value(row)}</span> }]
        : undefined;
    ...
    return (
        <CommonTable
            ...
            onRowDelete={rowDeleteCallback ? (row) => rowDeleteCallback(row) : undefined}
            onRowAction={actionCallback ? (row) => actionCallback(row) : undefined}
            className="extension-tab-table-wrapper"
        />
    );
};
const ScrollTable = React.memo(
    ({
        ...
        return (
            <CommonTable
                data={pList}
                editable={!!saveCallback}
                showRowNumber
                infiniteScroll={{ onLoadMore: eocCallback, hasMore: hasMoreData }}
                onRowAction={actionCallback ? (row) => actionCallback(row) : undefined}
                onRowDelete={deleteCallback ? (row) => deleteCallback(row) : undefined}
                onSave={saveCallback ? (modInfo) => saveCallback(modInfo as any) : undefined}
                className="extension-tab-scroll-table-wrapper"
            />
        );
    }
);
