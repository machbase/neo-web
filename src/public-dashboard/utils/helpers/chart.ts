...
const isChartType = (fill?: number, stroke?: any) => {
    if (fill && fill >= 0 && stroke > 0) return 'area';
    else if (stroke > 0) return 'line';
    else if (stroke === 0) return 'point';
};
...
const getMinValue = (aSamples: any[]) => {
...
const getMaxValue = (aSamples: any[]) => {
    let sMax = 0;
...
const getCalculatedValue = (aSamples: any[], aCalculationType: string) => {
