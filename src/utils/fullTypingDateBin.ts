...
const CALENDAR_DAY_ORIGIN = 'FROM_TIMESTAMP(-32400000000000)';
const CALENDAR_DAY_BIN = (timeColumn: string) => `DATE_BIN('day', 1, ${timeColumn}, ${CALENDAR_DAY_ORIGIN})`;
const buildTimeValueExpression = (timeColumn: string, periodUnit = `'{{period_unit}}'`, periodValue = '{{period_value}}') =>
    `TO_TIMESTAMP(CASE ${periodUnit} WHEN 'day' THEN DATE_BIN('day', ${periodValue}, ${timeColumn}, ${CALENDAR_DAY_ORIGIN}) WHEN 'hour' THEN DATE_BIN('hour', ${periodValue}, ${timeColumn}, ${CALENDAR_DAY_BIN(
        timeColumn
    )}) WHEN 'min' THEN DATE_BIN('minute', ${periodValue}, ${timeColumn}, ${CALENDAR_DAY_BIN(timeColumn)}) ELSE DATE_BIN('second', ${periodValue}, ${timeColumn}, ${CALENDAR_DAY_BIN(
        timeColumn
    )}) END) / 1000000`;

export const FULL_TYPING_TIME_VALUE_TEMPLATE = buildTimeValueExpression('TIME');
...
const FULL_TYPING_WITHOUT_VAR_TIME_VALUE = buildTimeValueExpression('TIME', `'sec'`, '10');
...
export const buildFullTypingQuery = (blockInfo: any) => {
    ...
    const timeValueExpression = buildTimeValueExpression(timeColumn);
