...
export const explainEtc1 =
    'When executing the backup command, the backup type and backup destination path must be defined. To back up the entire database, you must specify “DATABASE”. To back up specific tables, specify “TABLE” as the backup type. When backing up specific tables, you must specify the table name. Requires BACKUP privilege on MACHBASEDB (Machbase 8.5+).';
