...
describe('queryTagJsonKeyData', () => {
    ...
    test('uses the qualified table without a TQL use directive and forwards cancellation', async () => {
        const controller = new AbortController();

        await queryTagJsonKeyData({ ...params, paths: ['[a]'], signal: controller.signal });

        expect(mockedFetchTql.mock.calls[0][0]).toContain('from MACHBASEDB.SYS.TAG');
        expect(mockedFetchTql).toHaveBeenCalledWith(expect.any(String), undefined, controller.signal);
    });
