...
describe('JsonKeyDetailModal', () => {
    ...
    it('keeps the detail open and explains when Tag Analyzer cannot accept every key', async () => {
        const paths = Array.from({ length: 13 }, (_, index) => `[key${index}]`);
        mockedQuery.mockResolvedValue({
            rows: [{ base: '2026-08-25 10:00:00.000', values: paths.map((_path, index) => index) }],
        });
        const onOpenTagAnalyzer = jest.fn();
        open({ paths, onOpenTagAnalyzer });
        await waitFor(() => expect(screen.getByRole('table')).toBeInTheDocument());
        ...
        expect(onOpenTagAnalyzer).not.toHaveBeenCalled();
        expect(screen.getByText('Tag Analyzer supports up to 12 keys. Reduce the selection and try again.')).toBeInTheDocument();
        expect(screen.getByRole('table')).toBeInTheDocument();
    });

    it('shows a handoff validation error without closing the detail', async () => {
        const onOpenTagAnalyzer = jest.fn(() => 'JSON key is too long for Tag Analyzer.');
        open({ onOpenTagAnalyzer });
        await waitFor(() => expect(screen.getByRole('table')).toBeInTheDocument());

        fireEvent.click(screen.getByRole('button', { name: /Open in Tag Analyzer/ }));

        expect(screen.getByText('JSON key is too long for Tag Analyzer.')).toBeInTheDocument();
        expect(screen.getByRole('table')).toBeInTheDocument();
    });
