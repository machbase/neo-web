...
This audit covers the tab components used by the panel editor settings tabs.
...
- Production files: `8`
- Largest hotspots:
  - `EditorAxesTab.tsx` (`632` lines): X-axis, Y-axis, Y2-axis, range rows, threshold rows, and Y2 tag assignment
  - `EditorTimeTab.tsx` (`139` lines): time tab view that renders the date pickers, quick ranges, and clear action
  - `useEditorTimeTabState.ts` (`205` lines): time-input buffers, parsing, boundary updates, quick ranges, and clear/reset behavior
  - `EditorDataTab.tsx` (`262` lines): tag list editing, tag removal, and add-tag modal orchestration
...
- `Bad`
  - `EditorAxesTab.tsx` is the biggest editor UI hotspot and owns too many separate sub-areas.
  - `EditorDataTab.tsx` still carries a large editable-row JSX block even after the legacy adapter dependency was removed.

- `Warning`
  - `useEditorTimeTabState.ts` now owns the time-range coordination cleanly, but it is the next file to watch if more editor-time rules are added.
  - `EditorTimeTab.tsx` is much clearer now, but it still relies on several inline callback props in the rendered date pickers and buttons.

- `Good`
  - `EditorTimeTab.tsx` now mostly renders the tab UI while `useEditorTimeTabState.ts` owns the time-input workflow.
  - `EditorTabContent.tsx`, `EditorGeneralTab.tsx`, and `PanelEditorSettings.tsx` each have one clear job.

## File Audit

### `EditorAxesTab.tsx` (`632` lines)
...
Functions:
- `EditorAxesTab` (`line 77`, `553` lines): renders all axis controls, derives Y2 option lists, defines row config data, and wires axis updates back into editor state. Needs edit: `Yes`; it is too large and owns too many UI sub-areas.
...
Audit note:
- This is the biggest single editor tab file.
...
Functions:
- `EditorDataTab` (`line 20`, `240` lines): owns add-tag modal state and renders the editable tag rows for the panel tag set. Needs edit: `Yes`; it is still focused, but the JSX block is large and row-level concerns are still inline.
...
- Best cleanup: consider extracting a small `TagRowEditor` component so the tab stops carrying the entire row layout inline.

### `EditorDisplayTab.tsx` (`216` lines)

Role: edits chart type, point visibility, legend visibility, point radius, fill opacity, and line thickness.

Functions:
- `EditorDisplayTab` (`line 32`, `182` lines): renders chart-type presets and the display-setting inputs for one panel. Needs edit: `No`.
...
### `EditorGeneralTab.tsx` (`112` lines)

Role: edits panel title, drag zoom behavior, and time-keeper behavior.

Functions:
- `EditorGeneralTab` (`line 18`, `92` lines): renders the small general-settings form. Needs edit: `No`.
...
- Good single-responsibility tab.

### `PanelEditorSettings.tsx` (`86` lines)

Role: renders the editor tab strip and the shared settings panel body that hosts the active tab component.
...
### `EditorTimeTab.tsx` (`139` lines)

Role: renders the panel time override tab and delegates time-input buffer and parsing workflow to a dedicated hook.

Functions:
- `EditorTimeTab` (`line 15`, `122` lines): renders the date pickers, quick ranges, and clear action for the panel time override while using the dedicated hook state. Needs edit: `No`.

Audit note:
- The main coordination logic now lives in `useEditorTimeTabState.ts`, which is a better ownership boundary.
...
### `useEditorTimeTabState.ts` (`205` lines)

Role: owns the time tab input buffers, parses user-entered values, builds normalized time config updates, and handles quick-range and clear actions.
...
- `useEditorTimeTabState` (`line 98`, `71` lines): owns the local input state and returns the handlers used by the time tab UI. Needs edit: `No`.
...
Audit note:
- This is a cleaner owner for the time-range workflow than the JSX-heavy tab component.
