...
- Responsibility: This folder owns editor section components for general, data, axes, display, and time settings.
- Scope note: Generated audit files, test files, markdown files, and TAZ files are excluded from the inventory.
- Largest direct file: `editor/sections/AxesSection.tsx` (632 lines)
- Helper hotspot: `editor/sections/useTimeRangeSectionState.ts` (10 named functions)

## Files

### `AxesSection.tsx`
- Path: `editor/sections/AxesSection.tsx`
- Lines: 632
- Role: Renders the editor section that changes axis flags, explicit ranges, thresholds, and Y2 assignments.
...
- Functions:
  - `AxesSection` (553 lines, line 77) - Configures axis behavior for the panel. Needs edit: Yes. This function is large enough that it is likely doing more than one thing.
...
### `DataSection.tsx`
- Path: `editor/sections/DataSection.tsx`
- Lines: 262
- Role: Renders the editor section that changes selected tags, aliases, and data-source fields.
- Similar files: None that are close enough to justify a merge right now.
- Combine note: No combine action is recommended at the moment.
- Needs edit: Yes. This file is large enough that it is worth checking for mixed responsibilities.
- Functions:
  - `DataSection` (240 lines, line 20) - Manages the tag list assigned to a panel. Needs edit: Yes. This function is large enough that it is likely doing more than one thing.
...
### `DisplaySection.tsx`
- Path: `editor/sections/DisplaySection.tsx`
- Lines: 216
- Role: Renders the editor section that changes chart display options and chart type.
- Similar files: None that are close enough to justify a merge right now.
- Combine note: No combine action is recommended at the moment.
- Needs edit: Yes. This file is large enough that it is worth checking for mixed responsibilities.
- Functions:
  - `DisplaySection` (182 lines, line 32) - Controls how the panel is drawn visually. Needs edit: Yes. This function is large enough that it is likely doing more than one thing.
...
- Lines: 93
- Role: Routes the active editor tab to the correct section component.
- Similar files: None that are close enough to justify a merge right now.
- Combine note: No combine action is recommended at the moment.
- Needs edit: No. The current responsibility looks focused enough for now.
- Functions:
  - `EditorTabContent` (71 lines, line 20) - Chooses which editor section to render for the active tab. Needs edit: Warning. This function is starting to accumulate multiple steps and should be watched for further growth.

### `GeneralSection.tsx`
- Path: `editor/sections/GeneralSection.tsx`
- Lines: 112
- Role: Renders the editor section that changes panel title and high-level display flags.
- Similar files: None that are close enough to justify a merge right now.
- Combine note: No combine action is recommended at the moment.
- Needs edit: No. The current responsibility looks focused enough for now.
- Functions:
  - `GeneralSection` (92 lines, line 18) - Edits the general panel behavior such as title, zoom support, and time-keeper usage. Needs edit: Yes. This function is large enough that it is likely doing more than one thing.
...
- Lines: 86
- Role: Renders the editor settings column that contains the tabbed section content.
- Similar files: None that are close enough to justify a merge right now.
- Combine note: No combine action is recommended at the moment.
- Needs edit: No. The current responsibility looks focused enough for now.
- Functions:
...
### `TimeRangeSection.tsx`
- Path: `editor/sections/TimeRangeSection.tsx`
- Lines: 139
- Role: Renders the editor section that changes panel time mode, quick times, and explicit time bounds.
...
- Needs edit: Yes. This file is large enough that it is worth checking for mixed responsibilities.
- Functions:
  - `TimeRangeSection` (122 lines, line 15) - Edits the panel-specific time range override. Needs edit: Yes. This function is large enough that it is likely doing more than one thing.

### `useTimeRangeSectionState.ts`
- Path: `editor/sections/useTimeRangeSectionState.ts`
- Lines: 205
- Role: Owns the local input buffers and update handlers for the time-range editor section.
...
- Functions:
  ...
  - `useTimeRangeSectionState` (71 lines, line 98) - Owns the local input buffers and update handlers for the time-range editor section. Needs edit: Warning. This function is starting to accumulate multiple steps and should be watched for further growth.
