# `usePanelRangeSelectionState`
Source: [usePanelRangeSelectionState.ts](/C:/_github_repos/neo-web/src/components/tagAnalyzer/panel/usePanelRangeSelectionState.ts)
...
## Local state
The hook stores `selectionState`, which contains:
- `isOpen`
- `startTime`
- `endTime`
- `seriesSummaries`
...
6. Save `selectionState` with selected times and summary rows.
...
## Return value
The hook returns:
- `selectionState`: state used by the popup and FFT modal.
- `handleSelection`: passed to the chart as the selection handler.
- `handleCloseSelection`: closes the popup and clears selection state.
