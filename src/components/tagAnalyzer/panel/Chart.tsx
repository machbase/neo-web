...
const Chart = ({
    ...
    const setValue : () => void  = () => {
        ...
        const tooltipConfig = {
            ...
            formatter: function () {
                return `<div>
                    <div style="minWidth:0px; paddingLeft:10px; fontSize:10px"><div style="color: #afb5bc">${String((this as any).x).includes('.')
                        ? new Date((this as any).x - getTimeZoneValue() * 60000).toISOString().replace('T', ' ').replace('Z', '') + '.' + String((this as any).x).split('.')[1]
                        : new Date((this as any).x - getTimeZoneValue() * 60000).toISOString().replace('T', ' ').replace('Z', '')}</div></div>
                    <br/>
