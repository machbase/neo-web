...
describe('gBoardListUpdater', () => {
    describe('getNextBoardListWithPersistedBoardInfo', () => {
        it('strips shared tab fields from the active persisted board snapshot', () => {
            ...
            const sBoardInfo = parseLoadedTaz(sBoard);
    ...
    describe('getNextBoardListWithSavedBoard', () => {
        it('replaces only the matching board entry', () => {
            const sBoards = [
                {
                    id: 'board-1',
                    name: 'before.taz',
            ...
            const sSavedBoard = {
                ...
                path: '/after/',
                type: 'taz',
                panels: [{ meta: { panelKey: 'panel-1' } }],
                savedCode: 'saved panels',
                code: '',
            };

            expect(getNextBoardListWithSavedBoard(sBoards, sSavedBoard as any)).toEqual([
                sSavedBoard,
                sBoards[1],
            ]);
        });
    ...
    describe('getNextBoardListWithAppendedPersistedPanel', () => {
        it('appends the persisted panel only to the target board', () => {
            const sBoards = [
                {
                    id: 'board-1',
                    type: 'taz',
                    panels: [{ meta: { panelKey: 'existing-panel' } }],
                },
                {
                    id: 'board-2',
                    type: 'taz',
            ...
            const sPanel = {
                meta: {
                    panelKey: 'panel-2',
                    chartTitle: 'Created Panel',
                },
            } as any;

            expect(
                getNextBoardListWithAppendedPersistedPanel(sBoards, 'board-1', sPanel),
            ).toEqual([
                {
                    id: 'board-1',
                    type: 'taz',
                    version: TAZ_FORMAT_VERSION,
                    panels: [{ meta: { panelKey: 'existing-panel' } }, sPanel],
                },
                {
                    id: 'board-2',
                    type: 'taz',
