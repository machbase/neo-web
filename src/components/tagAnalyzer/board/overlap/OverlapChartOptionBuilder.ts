...
function resolveOverlapChartXAxisRanges(
    ...
    const sPadding = Math.max(
        getTimeRangeWidth(sDataRange) * OVERLAP_EMPTY_X_AXIS_PADDING_RATIO,
...
function formatOverlapTooltip(
    ...
    const sRows = sList
        .map((raw) => {
            ...
            return `<div style="color:${sColor}">${sName} : ${sXAxisLabel} : ${sValue?.[1] ?? ''}</div>`;
        })
        .join('<br/>');
...
export function buildOverlapChartOption(
    ...
    return {
        ...
        tooltip: buildOverlapTooltipOption(
            overlapChartInput.seriesData,
            overlapChartInput.isNumericXAxis,
        ),
        xAxis: {
