...
import { toStoredTimeValue } from './StoredTimeRangeAdapter';
...
describe('TimeBoundaryParsing', () => {
    ...
    describe('parseTimeRangeInputValue', () => {
        it('parses relative expressions and blank values into the structured holder', () => {
            expect(toStoredTimeValue(parseTimeRangeInputValue('')!)).toBe('');
            expect(toStoredTimeValue(parseTimeRangeInputValue('now-1h')!)).toBe('now-1h');
            expect(toStoredTimeValue(parseTimeRangeInputValue('last-30m')!)).toBe('last-30m');
            expect(toStoredTimeValue(parseTimeRangeInputValue('Now-1h')!)).toBe('Now-1h');
        });

        it('converts valid absolute dates into epoch milliseconds', () => {
            expect(toStoredTimeValue(parseTimeRangeInputValue(ABSOLUTE_TIME_TEXT)!)).toBe(
