...
} from './ChartHighlightSeriesConstants';
...
function getHighlightLabelData(aHighlights: PanelHighlight[], aLabelY: number) {
    return (aHighlights ?? []).filter(isRenderableHighlight).map((aHighlight, aIndex) => ({
        name: aHighlight.text || 'unnamed',
        value: [(aHighlight.timeRange.startTime + aHighlight.timeRange.endTime) / 2, aLabelY],
