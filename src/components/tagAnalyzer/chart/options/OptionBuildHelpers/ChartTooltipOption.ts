 ...
 * @param aParams The formatter params provided by ECharts.
 * @returns The tooltip HTML.
 */
function formatChartTooltip(aParams: TopLevelFormatterParams): string {
    const sMainSeriesItems = getMainSeriesTooltipItems(aParams);
 ...
 */
function getMainSeriesTooltipItems(aParams: TopLevelFormatterParams): EChartTooltipParam[] {
    return normalizeTooltipParams(aParams).filter((aItem) =>
 ...
 */
function normalizeTooltipParams(aParams: TopLevelFormatterParams): EChartTooltipParam[] {
    const sParams = Array.isArray(aParams) ? aParams : [aParams];

    return sParams.map(toChartTooltipParam);
}
 ...
 * @param aParam The ECharts formatter param.
 * @returns The local tooltip param shape.
 */
function toChartTooltipParam(aParam: ChartTooltipCallbackParam): EChartTooltipParam {
    return {
        axisValue: aParam.axisValue,
        color: getTooltipColor(aParam.color),
        seriesId: aParam.seriesId,
        seriesIndex: aParam.seriesIndex,
        seriesName: aParam.seriesName,
        value: getTooltipValue(aParam.value),
    };
}
 ...
 * @param aItem The normalized tooltip param.
 * @returns The tooltip row HTML.
 */
function formatTooltipRow(aItem: EChartTooltipParam): string {
    const sColorStyle = aItem.color ? `color:${aItem.color};` : '';

    return `<div style="${sColorStyle}margin:0;padding:0;white-space:nowrap">${aItem.seriesName} : ${aItem.value?.[1] ?? ''}</div>`;
}
 ...
 * @param aColor The ECharts callback color value.
 * @returns The CSS color string, if one exists.
 */
function getTooltipColor(aColor: CallbackDataParams['color']): string | undefined {
    return typeof aColor === 'string' ? aColor : undefined;
}
 ...
 * @param aValue The ECharts callback value.
 * @returns The normalized tooltip value, if it has the expected primitive shape.
 */
function getTooltipValue(aValue: CallbackDataParams['value']): EChartTooltipValue | undefined {
    return isTooltipValue(aValue) ? aValue : undefined;
}
 ...
 */
function isTooltipValue(aValue: CallbackDataParams['value']): aValue is EChartTooltipValue {
    return Array.isArray(aValue) && aValue.every(isTooltipValueItem);
}
 ...
 * @param aValue The candidate tooltip value item.
 * @returns Whether the item can be rendered by the tooltip.
 */
function isTooltipValueItem(aValue: unknown): aValue is TooltipValueItem {
    return aValue === undefined || typeof aValue === 'number' || typeof aValue === 'string';
}
