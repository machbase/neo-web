...
import { getEnclosingRange, getRangeCenter } from '../rangeExpression/rangeArithmetic';
import { PANEL_NAVIGATOR_DATA_X_AXIS_INDEX, PANEL_NAVIGATOR_Y_AXIS_INDEX } from './chartLayout';
import type { ChartHighlight } from './chartModel';
import {
...
function getHighlightAreaData(
    ...
    return highlights.map(
        (highlight): [HighlightAreaPoint, HighlightAreaPoint] => [
            {
                ...(includeName ? { name: highlight.text } : {}),
                xAxis: highlight.range.start,
                itemStyle: {
    ...
    return highlights.length > 0
        ? [{
              ...
              data: highlights.map((highlight, highlightIndex) => ({
                  name: highlight.text,
                  value: [getRangeCenter(highlight.range), sAxisMax - sLabelPadding],
                  highlightIndex,
