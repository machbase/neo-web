...
export async function fetchTableSchema(
    ...
    try {
        ...
        const sErrorMessage = getUnknownErrorMessage(
            error,
            TABLE_SCHEMA_REQUEST_FAILED_MESSAGE,
        );
...
function buildTableSchemaBatchSql(targets: TableSchemaBatchTarget[]): string {
    const sCaseBranches = targets
        .map(({ table, target }) => (
            `WHEN ${buildTableSchemaTargetCondition(target)} ` +
            `THEN ${buildSqlStringLiteral(table)}`
        ))
        .join(' ');
    ...
    return joinSqlLines([
        `SELECT CASE ${sCaseBranches} END AS TABLE_KEY,`,
        '       SYSTEM_COLUMNS.NAME,',
        '       SYSTEM_COLUMNS.TYPE,',
        '       SYSTEM_COLUMNS.FLAG',
        'FROM M$SYS_TABLES SYSTEM_TABLES,',
        ...
        '  AND SYSTEM_TABLES.USER_ID = SYSTEM_USERS.USER_ID',
        "  AND SYSTEM_COLUMNS.NAME <> '_RID'",
        `  AND (${sTargetConditions})`,
        'ORDER BY TABLE_KEY, SYSTEM_COLUMNS.ID',
    ]);
}

function buildTableSchemaTargetCondition(target: TableSchemaTarget): string {
    return joinSqlLines([
        `SYSTEM_USERS.NAME = UPPER(${buildSqlStringLiteral(target.userName)})`,
        `AND SYSTEM_COLUMNS.DATABASE_ID = ${target.databaseIdQuery}`,
        `AND SYSTEM_TABLES.NAME = ${buildSqlStringLiteral(target.tableName)}`,
    ]).replace(/\n/g, ' ');
}
