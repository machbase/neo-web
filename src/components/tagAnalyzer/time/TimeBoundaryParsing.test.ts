...
describe('TimeBoundaryParsing', () => {
    ...
    describe('parseTimeRangeInputValue', () => {
        it('parses relative expressions and blank values into the structured holder', () => {
            expect(serializeTimeBoundaryValue(parseTimeRangeInputValue('')!)).toBe('');
            expect(serializeTimeBoundaryValue(parseTimeRangeInputValue('now-1h')!)).toBe('now-1h');
            expect(serializeTimeBoundaryValue(parseTimeRangeInputValue('last-30m')!)).toBe('last-30m');
            expect(serializeTimeBoundaryValue(parseTimeRangeInputValue('Now-1h')!)).toBe('Now-1h');
        });

        it('converts valid absolute dates into epoch milliseconds', () => {
            expect(serializeTimeBoundaryValue(parseTimeRangeInputValue(ABSOLUTE_TIME_TEXT)!)).toBe(
