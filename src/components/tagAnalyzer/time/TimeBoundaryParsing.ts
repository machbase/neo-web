...
export function normalizeTimeBoundaryInputValues(
    startValue: TimeBoundaryInputValue | undefined,
    endValue: TimeBoundaryInputValue | undefined,
): ResolvedTimeBounds {
    return normalizeTimeRangeConfig({
        start: parseTimeBoundaryInputValue(startValue),
        end: parseTimeBoundaryInputValue(endValue),
    });
}
...
export function serializeTimeBoundaryValue(boundary: TimeBoundary): TimeBoundaryInputValue {
    switch (boundary.kind) {
        case 'empty':
            return '';
        case 'absolute':
...
export function toTimeBoundaryRangeInput(
    source: TimeRangeConfig | ResolvedTimeBounds,
): TimeBoundaryRangeInput {
    const sRangeConfig = 'rangeConfig' in source ? source.rangeConfig : source;

    return {
        bgn: serializeTimeBoundaryValue(sRangeConfig.start),
        end: serializeTimeBoundaryValue(sRangeConfig.end),
    };
}
...
function parseTimeBoundaryInputValue(
    value: TimeBoundaryInputValue | undefined,
): TimeBoundary {
    ...
    return parseTimeRangeInputValue(value) ?? { kind: 'raw', value: value };
}
