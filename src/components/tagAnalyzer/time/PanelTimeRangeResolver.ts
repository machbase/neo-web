...
export async function resolvePanelTimeRange(
    ...
    if (isEdit && mode === 'reset') {
        return resolveEditResetPanelRange(
            boardTime,
            panelTime,
            timeBoundaryRanges,
        );
    ...
    if (isEdit && mode === 'initialize') {
        return resolveEditInitializePanelRange(
            boardTime,
            panelData,
            panelTime,
            timeBoundaryRanges,
    ...
    if (mode === 'reset') {
        return resolveViewResetPanelRange(
            boardTime,
            panelData,
            panelTime,
            timeBoundaryRanges,
    ...
    return resolveViewInitializePanelRange(
        boardTime,
        panelData,
        panelTime,
        timeBoundaryRanges,
 ...
 * @param {Pick<PanelTime, 'rangeConfig' | 'defaultRange'>} panelTime - The raw panel time payload.
 * @returns {PanelTimeRangeSource} The normalized panel time range source.
 */
export function normalizePanelTimeRangeSource(
    panelTime: Pick<PanelTime, 'rangeConfig' | 'defaultRange'>,
): PanelTimeRangeSource {
    const sDefaultRange = panelTime.defaultRange;

    return {
        range: toConcreteTimeRange(panelTime.rangeConfig),
...
/**
 * Resolves the edit initialize range.
 * Intent: Keep edit initialization on the last fetched board bounds first, then the normal panel rule chain.
 * @param {InputTimeBounds} boardTime - The board time input.
 * @param {PanelData} panelData - The panel data payload.
 * @param {PanelTime} panelTime - The panel time payload.
 * @param {ValueRangePair | null} timeBoundaryRanges - The fetched boundary ranges.
 * @returns {Promise<TimeRangeMs>} The resolved edit initialize range.
 */
async function resolveEditInitializePanelRange(
    boardTime: InputTimeBounds,
    panelData: PanelData,
    panelTime: PanelTime,
    timeBoundaryRanges: ValueRangePair | null,
): Promise<TimeRangeMs> {
    const sEditBoardLastRange = normalizeEditBoardLastRange(timeBoundaryRanges);
    if (sEditBoardLastRange) {
        return sEditBoardLastRange;
    }

    return resolvePanelRangeFromSharedRules(
        boardTime,
        panelData,
        panelTime,
        false,
...
/**
 * Resolves the edit reset range.
 * Intent: Keep reset editing pinned to the current preview bounds or the current panel/board range.
 * @param {InputTimeBounds} boardTime - The board time input.
 * @param {PanelTime} panelTime - The panel time payload.
 * @param {ValueRangePair | null} timeBoundaryRanges - The fetched boundary ranges.
 * @returns {TimeRangeMs} The resolved edit reset range.
 */
function resolveEditResetPanelRange(
    boardTime: InputTimeBounds,
    panelTime: PanelTime,
    timeBoundaryRanges: ValueRangePair | null,
): TimeRangeMs {
    const sEditPreviewRange = normalizeEditPreviewTimeRange(timeBoundaryRanges);
    if (sEditPreviewRange) {
        return sEditPreviewRange;
    }
...
/**
 * Resolves the view initialize range.
 * Intent: Keep non-edit initialization on the board last-range first, then normal panel rules.
 * @param {InputTimeBounds} boardTime - The board time input.
 * @param {PanelData} panelData - The panel data payload.
 * @param {PanelTime} panelTime - The panel time payload.
 * @param {ValueRangePair | null} timeBoundaryRanges - The fetched boundary ranges.
 * @returns {Promise<TimeRangeMs>} The resolved view initialize range.
 */
async function resolveViewInitializePanelRange(
    boardTime: InputTimeBounds,
    panelData: PanelData,
    panelTime: PanelTime,
    timeBoundaryRanges: ValueRangePair | null,
): Promise<TimeRangeMs> {
    const sBoardLastRange = normalizeBoardLastRange(boardTime, timeBoundaryRanges);
    if (sBoardLastRange) {
        return sBoardLastRange;
    }
...
/**
 * Resolves the view reset range.
 * Intent: Keep non-edit reset on the board last-range first, then explicit panel rules, then the default board fallback.
 * @param {InputTimeBounds} boardTime - The board time input.
 * @param {PanelData} panelData - The panel data payload.
 * @param {PanelTime} panelTime - The panel time payload.
 * @param {ValueRangePair | null} timeBoundaryRanges - The fetched boundary ranges.
 * @returns {Promise<TimeRangeMs>} The resolved view reset range.
 */
async function resolveViewResetPanelRange(
    boardTime: InputTimeBounds,
    panelData: PanelData,
    panelTime: PanelTime,
    timeBoundaryRanges: ValueRangePair | null,
    ...
    return resolvePanelRangeFromSharedRules(
        ...
        panelTime,
        true,
        () => resolveConcreteRangeFallback(getDefaultBoardRange(boardTime, panelTime), timeBoundaryRanges),
    );
}
    ...
    return setTimeRange(
        {
            range: undefined,
            defaultRange: {
                startTime: panelTime.defaultRange?.min ?? 0,
                endTime: panelTime.defaultRange?.max ?? 0,
            },
        },
        normalizeBoardTimeRangeInput(boardTime),
    );
...
function normalizeAbsolutePanelRange(panelTime: PanelTime): TimeRangeMs | undefined {
    if (!isAbsoluteTimeRangeConfig(panelTime.rangeConfig)) {
        return undefined;
    }

    return toConcreteTimeRange(panelTime.rangeConfig);
}
...
function normalizeNowPanelRange(
    ...
    if (!isNowRelativeTimeRangeConfig(panelTime.rangeConfig)) {
        return undefined;
    }
...
async function getRelativePanelLastRange(
    ...
    if (
        boardTime.kind !== 'resolved' ||
        !isLastRelativeTimeRangeConfig(panelTime.rangeConfig) ||
        !isRelativeTimeRangeConfig(boardTime.value.rangeConfig)
    ) {
        return undefined;
    }

    const sBoardRangeInput = toTimeBoundaryRangeInput(boardTime.value.rangeConfig);
    const sPanelRangeInput = toTimeBoundaryRangeInput(panelTime.rangeConfig);
    ...
    return resolveLastRelativeTimeRange(sTimeRange.end.max, panelTime.rangeConfig);
}
 ...
 * @param {PanelData} panelData - The panel data payload.
 * @param {PanelTime} panelTime - The panel time payload.
 * @param {boolean} includeAbsolutePanelRange - Whether absolute panel ranges should be considered.
 * @param {() => TimeRangeMs} fallbackRange - The fallback range resolver.
 * @returns {Promise<TimeRangeMs>} The resolved panel range.
 */
async function resolvePanelRangeFromSharedRules(
    boardTime: InputTimeBounds,
    panelData: PanelData,
    panelTime: PanelTime,
 ...
 * @param {TimeRangeMs} baseRange - The direct fallback range candidate.
 * @param {ValueRangePair | null} timeBoundaryRanges - The fetched time boundaries for the current panel tags.
 * @returns {TimeRangeMs} The resolved fallback range.
 */
