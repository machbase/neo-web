...
request.interceptors.request.use(
    ...
    (config: AxiosRequestConfig) => {
        ...
        // } else sHeaders.Authorization = `Bearer ${localStorage.getItem(LocalStorageKeys.ACCESS_TOKEN)}`;
        // return config;
        // }
    },
    (error: AxiosError) => {
...
request.interceptors.response.use(
    ...
    async (error: AxiosError) => {
        ...
        // RefreshToken === true
        // if (sRefreshToken) {
        //     localStorage.setItem('accessToken', sRefreshToken.accessToken);
        //     localStorage.setItem('refreshToken', sRefreshToken.refreshToken);
        ...
        //     }
        //     if (error.response.config.url !== AUTH_API.LOGIN) {
